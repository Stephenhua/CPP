...
//递归进行求解；
bool isSymantecTreeCore(BinaryTreeNode* pRoot1 , BinaryTreeNode* pRoot2){
    ...
    return isSymantecTreeCore(pRoot1->m_pLeft ,pRoot2->m_pRight) && isSymantecTreeCore(pRoot2->m_pRight ,pRoot1->m_pLeft);

}
//思路，前序遍历 和对称前序进行比较，如果两个相等则，表明其是二叉对称数；
bool isSymantecTree( BinaryTreeNode* pRoot){

    return isSymantecTreeCore( pRoot ,pRoot);
}
...
void Test(char *testname,BinaryTreeNode* pRoot, bool expected){
    ...
    if(isSymantecTree(pRoot)==expected){
        printf("Passed.\n");
    }
    if(isSymantecTree(pRoot)!=expected){
        printf("failed.\n");
